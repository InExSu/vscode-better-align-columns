...
2026-05-19 11-32-01
6.20.0
Хорошо выровненный код:
    match ($ns->s_FTP_From) {
        FTP_From::FTP  => FTP_Files_Read($ns)  ,                     
        FTP_From::Local=> Local_Files_Read($ns),                     
        FTP_From::B24  => $ns->FTP_Files_Read = [],
    };
    не должен выравниваться так
    match ($ns->s_FTP_From) {
        FTP_From::FTP  => FTP_Files_Read($ns)  ,                     
        FTP_From::Local=> Local_Files_Read($ns),                     
        FTP_From::B24  => $ns                  -> FTP_Files_Read = [],
    };
 - между $ns и -> не нужно выравнивание - потому что в строке выше другой признак выравнивания - запятая.    
 Так указано в AI Prompt math 2 TS.md:
...
2026-05-29 13-05-59
src/extension.ts
Проблема - расширение выравнивает внутри ''.
Из такого кода:
...
Получается:
  ...
  UNIX   : '02.02                           ; 02.04.04\n\nUNIX (байонет) + Unix Air', 
  SS_F   : '03.01 Самоспасатели Фильтрующие', 
Нужно уметь определять строковые литералы и внунтри них не выравнивать.  
Создай тест для этого случая.
Не сломай другие тесты.
Собери новую версию vsix, установи vsix в vs code, запусти gh.sh
...
НО запятые должны быть выровнениы все, а я вижу что после '<='  , они перестали выравниваться по столбцу
Создай тест для этого случая.
Исправь существующие тесты.
Собери новую версию vsix, установи vsix в vs code, придумай описание коммита,запусти gh.sh.
Обычные действия выполняй сам.
После успешных действий перегрузи vs code.



2026-06-12 02-00-17
Исследуй расширение начиная с src/extension.ts и исправь проблему:
На больших файлах 1500 строк, выделяю весь код, вызываю расширение, оно выравнивает код первые строк 300 а следующие строки не выравнивает. 
Хотя в этих строках есть код, который может выровняться расширением.
Придумай коммит, запусти gh.sh, установи новую версию расширения без вопросов мне.